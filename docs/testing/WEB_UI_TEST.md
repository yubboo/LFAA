...
1. 顶栏只显示 `Web 工作台`、更多、分享等页面级操作；
2. 顶栏不再显示左栏 / 终端 / 右栏三个 Shell Actions；
3. 左栏按钮固定在**中间主区左上角**；
4. 终端 / 右栏按钮固定在**中间主区右上角**；
5. 左右栏正式展开 / 收起时，三个按钮仍属于中间主区，不跟随侧栏内部内容移动；
6. 窄屏可以隐藏分享，但 Shell Actions 必须仍可操作。
...
6. 使用中间主区右上角终端按钮、`Ctrl+J` 或右栏“终端”入口重新展开；
7. 终端 Dock 不应挤坏主对话区。
