...
const slider=read("packages/client/ui/src/ui-controls/DiscreteSlider.tsx");
const sliderCss=read("packages/client/ui/src/ui-controls/discrete-slider.css");
const effectHost=read("packages/client/ui/src/ui-effects/UiEffectHost.tsx");
const effectCss=read("packages/client/ui/src/ui-effects/effects.css");
const particleCanvas=read("packages/client/ui/src/ui-effects/ParticleStreamCanvas.tsx");
const reasoningControl=read("packages/client/app-shell/src/reasoning-control.ts");
const service=read("packages/settings/config-system/src/settings/ai/core/account-service.ts");
const bridge=read("packages/api/settings-controller/src/ai-config-bridge.ts");
const runtime=read("packages/core/agent-runtime/src/core/contracts.ts");
const codexAppServer=read("packages/harness/codex-app-server/src/codex-app-server.ts");
const aiController=read("packages/client/app-shell/src/workbench/settings/logic/useAiSettingsController.ts");
const sessionController=read("packages/client/workspace/src/shared/logic/useWorkspaceSessionController.ts");
...
test("agent run model binding carries validated runtime settings through settings -> session",()=>{assert.match(runtime,/readonly settings\?: Readonly<Record<string, string \| number \| boolean>>/);assert.match(aiController,/settings:\s*activeAccount\?\.modelSettings\s*\?\?\s*\{\}/);assert.match(sessionController,/settings:\s*\{\s*\.\.\.\(activeModelBinding\.settings\s*\?\?\s*\{\}\),\s*\.\.\.modelSettingOverrides\s*\}/);});
