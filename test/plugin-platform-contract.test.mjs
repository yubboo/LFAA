...
test("plugin sdk owns canonical capability and app-pack contracts", () => {
  const source = read("packages/plugin-sdk/src/contracts.ts");
...
test("plugin runtime owns generation registry instead of execution", () => {
  const source = read("packages/plugin-runtime/src/registry.ts");
...
test("agent runtime reuses plugin capability vocabulary", () => {
  const source = read("packages/agent-runtime/src/core/contracts.ts");
...
test("language ownership gate protects TS product plane and Rust kernel", () => {
  ...
  assert.match(source, /walk\("crates"/);
...
test("plugin manifests declare credential needs without carrying secret values", () => {
  const contract = read("packages/plugin-sdk/src/contracts.ts");
  const registry = read("packages/plugin-runtime/src/registry.ts");
  const credentials = read("packages/credentials/src/index.ts");
...
test("plugin package manager is isolated from browser/runtime contracts", () => {
  const lifecycle = read("packages/plugin-runtime/src/lifecycle.ts");
  const host = read("packages/plugin-host-node/src/index.ts");
  ...
  assert.match(host, /\.lfaa.*state.*plugin-profile/s);
...
test("plugin install source is inspected before transactional install", () => {
  const parser = read("packages/plugin-runtime/src/install-spec.ts");
  const lifecycle = read("packages/plugin-runtime/src/lifecycle.ts");
