...
test("workspace is one real parent package with chat work manual shared children", () => {
  ...
  assert.equal(exists(`${workspaceRoot}/manual/index.ts`), true);
...
test("workspace public entry exports modes and shared session only through child public entries", () => {
  ...
  for (const token of ['./chat', './work', './manual', './shared']) assert.match(entry, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  assert.doesNotMatch(entry, /\/view\/|\/logic\/|\/styles\/|\/contracts\//);
});

test("chat work and manual keep their product views inside child modules", () => {
  ...
  assert.equal(exists(`${workspaceRoot}/manual/view/ManualWorkspace.tsx`), true);
  assert.equal(exists(`${workspaceRoot}/manual/styles/ManualWorkspace.module.css`), true);
...
test("workspace session is the single Chat Work run owner while Manual stays outside Agent Runtime", () => {
  ...
  assert.match(session, /mode === "manual"/);
  assert.match(session, /automationReady:/);
