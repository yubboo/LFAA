...
必须把本文件视为执行合同，而不是参考建议。每次任务都要实际完成：
...
禁止只在回复里声称“会遵守”，但没有同步更新对应代码、文档、历史和验证记录。

旧版本不得覆盖；修复必须进入新版本，当前与历史必须分开。
...
- 使用结构化中文文件头；
- 写明作用 / 负责 / 不负责 / 状态归属 / 对外接口 / 关联文件 / 修改注意事项；
- 复杂算法解释“为什么这样做”和关键不变量；
- CSS 按页面区域 / 盒子结构分区，并写明与 TSX 的对应关系；
- 新增一级业务/技术目录时补 README 或项目地图入口。

禁止“代码能运行就算完成”。

Windows PowerShell 关键脚本额外要求：
...
原因：根 BAT 使用 Windows `powershell.exe`，PowerShell 5.1 对无 BOM UTF-8 文件识别不可靠。补中文注释、格式化或批量重写时不得移除 BOM。
