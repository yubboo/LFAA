...
- Workbench 页面必须遵守父子级区域边界：`AgentWorkbench` 只做 Composition Root；Left / Center / Right / BottomTerminal 为独立大模块；Center 负责 Header / Workspace Mode View / Composer；Composer 的 Runtime Control 独立。局部修复默认只允许修改对应子模块和明确共享 Primitive，禁止顺手编辑兄弟区域。
...
- `packages/ui` 定位为 UI Kit / Design System / Shared Interaction Engine；TS/TSX 可实现 DOM/ARIA/Pointer/Canvas 等通用 UI 行为，但不得承载 LFAA 产品业务语义。`packages/workspace` 拥有 Chat/Work 两种工作模式与共享 Workspace Session；`packages/app-shell` 拥有 Shell/Left/Center Chrome/Composer/RuntimeControl/Right/Terminal/Settings 等产品外壳。
...
| `packages/ui/` | UI Kit / Design System / Shared Interaction Engine；通用控件、布局、Motion、Effect、Canvas Renderer/Interaction 与可复用 Feature UI | LFAA 产品页面 Owner、Provider 网络请求、Secret、Config/Session 产品真值、宿主专属桥 |
