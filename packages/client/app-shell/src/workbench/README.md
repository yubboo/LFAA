...
- 改 Chat Mode → `packages/client/workspace/src/chat/**`。
- 改 Work Canvas 产品行为 → `packages/client/workspace/src/work/**`；若是通用 InfiniteCanvas Pointer 算法才进入 `@lfaa/ui`。
- 改输入框 / 模型推理卡 → `packages/client/app-shell/src/workbench/center/composer/**`。
