/**
 * 文件：packages/plugin-host-node/src/index.ts
 ...
 * 状态归属：Profile package.json/pnpm-lock.yaml 是已安装包事实；lfaa.profile.enabled 是启用事实。
...
const PROFILE_RELATIVE = path.join(".lfaa", "state", "plugin-profile");
...
async function withProfileLock<T>(dir: string, operation: () => Promise<T>): Promise<T> {
  ...
  const lockFile = path.join(dir, ".lfaa-plugin-profile.lock");
...
export class NodePluginPackageHost implements PluginPackageHostPort {
  ...
  constructor(options: NodePluginPackageHostOptions) {
    ...
    this.#profileDir = path.resolve(options.profileDir ?? path.join(this.#projectRoot, PROFILE_RELATIVE));
  ...
  async #inspectWithStaging(spec: ParsedPluginInstallSpec, signal?: AbortSignal): Promise<PluginSpecInspection> {
    const tmpRoot = path.join(this.#projectRoot, ".lfaa", "tmp");
  ...
  async install(
    ...
    try {
      return await withProfileLock(this.#profileDir, async () => {
        ...
        const logDir = path.join(this.#projectRoot, ".lfaa", "logs", "plugin-manager");
