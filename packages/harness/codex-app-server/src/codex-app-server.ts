...
export class CodexAppServerTextRuntime {
  ...
  async steerText(sessionKey: string, input: string): Promise<string> {
    const active = this.#activeTurns.get(sessionKey);
    if (!active) throw new Error("当前 Codex Session 没有可干预的进行中 Turn。");
    const text = input.trim();
    if (!text) throw new Error("干预内容为空。");
    const result = await this.#client.request("turn/steer", {
      threadId: active.threadId,
      input: [{ type: "text", text }],
      expectedTurnId: active.turnId,
    });
    if (!isRecord(result) || !stringField(result, "turnId")) throw new Error("Codex turn/steer 响应无效。");
    return stringField(result, "turnId")!;
  }
  ...
  async #runText(options: CodexTextRunInput): Promise<CodexTextRunResult> {
    ...
    try {
      ...
      this.#activeTurns.set(options.sessionKey, { threadId, turnId: expectedTurnId });
      ...
      const active = this.#activeTurns.get(options.sessionKey);
      if (active?.threadId === threadId && active.turnId === expectedTurnId) this.#activeTurns.delete(options.sessionKey);
      unsubscribe();
    }
  }
