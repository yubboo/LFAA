...
import type { AgentWorkbenchProps, DevResourceItem, LayoutMode, ResourceKind } from "../contracts";
import { RightShellActions } from "../shell";
...
export function RightSidebarRegion({ resources=[], resourceBridgeStatus="offline", layoutMode, terminalOpen, rightCollapsed, onToggleTerminal, onToggleRight }:RightSidebarRegionProps) {
  ...
  return <aside className={styles.root} data-layout-mode={layoutMode} data-ui="right-sidebar">
    ...
    <div className={styles.body}>
      <header className={styles.header}><div className={styles.headerPrimary}><strong>工具与资源</strong><span>当前项目</span></div><div className={styles.headerActions}><span className={styles.bridge} data-status={resourceBridgeStatus}><i/>{statusText}</span></div></header>
      ...
      <div className={styles.resourceHead}><span>项目资源</span><b className={styles.count}>{resources.length}</b></div>
      <div className={styles.resourceGroups}>{groups.map(({kind,items})=><section key={kind}><header className={styles.groupHeader}><span>{resourceLabels[kind]}</span><b className={styles.count}>{items.length}</b></header>{items.length===0?<p className={styles.empty}>.lfaa/{kind}</p>:items.map((item)=><div className={styles.resource} key={`${kind}:${item.relativePath}`}><WorkbenchIcon name={item.entryType==="directory"?"folder":"file"} size={15}/><div className={styles.resourceBody}><strong>{item.name}</strong><small>{item.relativePath}</small></div></div>)}</section>)}</div>
      <div className={styles.devNote}><strong>只读资源桥接</strong><p>资源舱只读；终端是单独的本地开发 PTY，会执行你亲自输入的命令。</p></div>
    </div>
  </aside>;
}
