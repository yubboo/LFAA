# Workbench 父子边界（v0.0.96 / #21.25）

> Workbench 是产品外壳，不再把 Chat/Work 两种 Workspace 投影、Session、Canvas 产品状态全部堆在 App Shell 内。父目录表示领域，子目录表示职责；禁止为了模块化过度拆成大量平级 package。
...
└─ workbench/
   ...
   ├─ left/        # 左侧栏
   ├─ center/      # Header + Workspace 公共投影 + Composer 的装配
   │  ├─ header/
   │  └─ composer/
   │     └─ runtime-control/
   ├─ right/
...
@lfaa/workspace
├─ chat/           # Chat Timeline / 线性投影
├─ work/           # Work / Infinite Canvas 产品投影和布局状态
...
- Composer/RuntimeControl 继续属于 App Shell；Chat/Work Run 与投影不复制到 Composer。
...
- 改 Chat 投影 → `packages/workspace/src/chat/**`。
