 ...
 * 关联文件：packages/client/app-shell/src/workbench/**、packages/client/workspace/src/**、packages/client/ui/src/ui-*、test/workbench-module-boundary.test.mjs、scripts/workspace-preflight.mjs。
...
const rootTsx=read("packages/client/app-shell/src/AgentWorkbench.tsx");
const chrome=read("packages/client/app-shell/src/workbench/shell/logic/useWorkbenchChromeController.ts");
const shell=read("packages/client/app-shell/src/workbench/shell/view/WorkbenchShell.tsx");
const shellCss=read("packages/client/app-shell/src/workbench/shell/styles/WorkbenchShell.module.css");
const shellButton=read("packages/client/app-shell/src/workbench/shell/view/ShellHeaderButton.tsx");
const rightShellActions=read("packages/client/app-shell/src/workbench/shell/view/RightShellActions.tsx");
const shellButtonCss=read("packages/client/app-shell/src/workbench/shell/styles/ShellHeaderButton.module.css");
const themeCss=read("packages/client/app-shell/src/workbench/shell/styles/WorkbenchTheme.module.css");
...
const chatWorkspace=read("packages/client/workspace/src/chat/view/ChatWorkspace.tsx");
const workCanvas=read("packages/client/workspace/src/work/view/WorkWorkspace.tsx");
const conversationCss=read("packages/client/workspace/src/chat/styles/ChatWorkspace.module.css");
const composer=read("packages/client/app-shell/src/workbench/center/composer/view/ComposerRegion.tsx");
const composerCss=read("packages/client/app-shell/src/workbench/center/composer/styles/Composer.module.css");
const addMenu=read("packages/client/app-shell/src/workbench/center/composer/view/AddCapabilityMenu.tsx");
const permission=read("packages/client/app-shell/src/workbench/center/composer/view/PermissionControl.tsx");
const runtimeView=read("packages/client/app-shell/src/workbench/center/composer/runtime-control/view/RuntimeControl.tsx");
...
const globalCss=read("packages/client/app-shell/src/agent-workbench.css");
const allApp=[rootTsx,chrome,shell,shellButton,rightShellActions,left,center,header,chatWorkspace,workCanvas,composer,addMenu,permission,runtimeView,runtimeRow,runtimeController,runtimePicker,session].join("\n");
const layoutConfig=read("packages/client/ui/src/workbench/workbench-layout.config.ts");const resizeTsx=read("packages/client/ui/src/workbench/ResizableWorkbench.tsx");const workbenchCss=read("packages/client/ui/src/workbench/workbench.css");const interactionConfig=read("packages/client/ui/src/workbench/workbench-interaction.config.ts");
const sharedSliderTsx=read("packages/client/ui/src/ui-controls/DiscreteSlider.tsx");const sharedSliderCss=read("packages/client/ui/src/ui-controls/discrete-slider.css");const sharedEffectCss=read("packages/client/ui/src/ui-effects/effects.css");const sharedEffectHost=read("packages/client/ui/src/ui-effects/UiEffectHost.tsx");const particleCanvas=read("packages/client/ui/src/ui-effects/ParticleStreamCanvas.tsx");const sharedEffectRegistry=read("packages/client/ui/src/ui-effects/registry.ts");const sharedExtensionRegistry=read("packages/client/ui/src/ui-extension/registry.ts");const animatedDisclosure=read("packages/client/ui/src/ui-motion/AnimatedDisclosure.tsx");const shortcutHook=read("packages/client/ui/src/ui-shortcuts/useShortcut.ts");const damped=read("packages/client/ui/src/ui-resize/damped-motion.ts");const layers=read("packages/client/ui/src/ui-overlay/layers.css");
...
const moduleCssFiles=[];const walkCss=(dir)=>{for(const entry of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,entry.name);if(entry.isDirectory())walkCss(file);else if(entry.name.endsWith(".module.css"))moduleCssFiles.push(file);}};for(const cssRoot of ["packages/client/app-shell/src/workbench","packages/client/workspace/src"])walkCss(path.join(root,cssRoot));
...
for(const moduleIndex of ["left/index.ts","center/index.ts","center/header/index.ts","center/composer/index.ts","center/composer/runtime-control/index.ts","right/index.ts","terminal/index.ts","shell/index.ts","settings/index.ts","shared/index.ts"])if(!fs.existsSync(path.join(root,"packages/client/app-shell/src/workbench",moduleIndex)))fail(`missing public module boundary ${moduleIndex}`);
for(const moduleIndex of ["chat/index.ts","work/index.ts","shared/index.ts"])if(!fs.existsSync(path.join(root,"packages/client/workspace/src",moduleIndex)))fail(`missing workspace public module boundary ${moduleIndex}`);
if(fs.existsSync(path.join(root,"packages/client/app-shell/src/workbench/center/conversation"))||fs.existsSync(path.join(root,"packages/client/app-shell/src/workbench/session")))fail("Chat/Work/Session must stay under @lfaa/workspace, not app-shell legacy owners");
...
const reasoningControl=read("packages/client/app-shell/src/reasoning-control.ts");
